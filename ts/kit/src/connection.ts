...
import { getWritableAccounts, isRouter, parseCommitsLogsMessage, parseScheduleCommitsLogsMessage } from "./utils";
...
export class Connection {
    ...
    public static async create(
        ...
        return new Connection(clusterUrlHttp, clusterUrlWs ?? clusterUrlHttp.replace(/^http/, "ws"), rpc, rpcSubscriptions, isRouterResult);
    }
    ...
    public async confirmTransaction(
        ...
        const getRecentSignatureConfirmationPromise = createRecentSignatureConfirmationPromiseFactory(solanaRpc);
     ...
     * ```
     */
    public async prepareTransactionWithLatestBlockhash (transaction: TransactionMessage & TransactionMessageWithFeePayer) {
    ...
    public async sendTransaction(
        ...
        const hasBlockhash =
            ...
            (transaction.lifetimeConstraint as { blockhash?: unknown }).blockhash !== undefined;

        if (!hasBlockhash) {
            transaction = await this.prepareTransactionWithLatestBlockhash(transaction);
        }
        const isAlreadySigned = (transaction as any).signatures &&Object.keys((transaction as any).signatures).length > 0;
        const signedTransaction = isAlreadySigned &&
            "messageBytes" in transaction &&
            "signatures" in transaction
            ? transaction as Readonly<{ messageBytes: TransactionMessageBytes; signatures: SignaturesMap; }>
            : await this.partiallySignTransaction(signers, transaction);
        const serializedTransaction = getBase64EncodedWireTransaction(signedTransaction);
        const signature = await this.rpc
            .sendTransaction(serializedTransaction, { encoding: "base64", skipPreflight, preflightCommitment, ...options })
            .send();
        return signature;
    }
    ...
    ) : Promise<Readonly<TransactionWithBlockhashLifetime & Readonly<{ messageBytes: TransactionMessageBytes; signatures: SignaturesMap; }>>>{
        assertIsTransactionMessageWithBlockhashLifetime(transaction);
        const compiledTransaction = compileTransaction(transaction);
        const signedTransaction = await partiallySignTransaction(signers, compiledTransaction);
    ...
    public async getLatestBlockhashForTransaction(
        ...
        throw new Error(`Invalid blockhash response: ${JSON.stringify(blockhashResponse)}`);
        }
    ...
    public async getCommitmentSignature(
        ...
        const txSchedulingSgn = await this.rpc.getTransaction(signature, { encoding: "base64",  maxSupportedTransactionVersion: 0 }).send()
        ...
        const txCommitInfo = await this.rpc.getTransaction(scheduledCommitSgn, { encoding: "base64",  maxSupportedTransactionVersion: 0 }).send()
