//! Minimal Pinocchio counter demonstrating the Ephemeral Rollups lifecycle where the
//! commit / commit-and-undelegate steps are issued through the **intent bundle**
//! builder (`MagicIntentBundleBuilder`) instead of the standalone `commit_accounts`
//! helpers. A bundle can aggregate commits, undelegations and post-commit actions
//! into a single magic-program instruction; here we use the minimal single-commit and
//! single-commit-and-undelegate forms.
//!
...
/// Schedule a commit (and optionally undelegate) of the counter from the ER, built as
/// a magic intent bundle.
fn commit(accounts: &[AccountView], and_undelegate: bool) -> ProgramResult {
    ...
    // Scratch buffer the bundle is serialized into before the CPI.
    ...
    if and_undelegate {
        ...
        builder.commit(committed).build_and_invoke(&mut data_buf)
    }
}
