/// Commit and undelegate permission instruction builder
use crate::access_control::pinocchio::instructions::COMMIT_AND_UNDELEGATE_DISCRIMINATOR;
...
impl CommitAndUndelegatePermissionBuilder {
    ...
    pub fn instruction<'a>(
        ...
        commit_and_undelegate_permission_instruction_impl(
...
/// Build a commit_and_undelegate_permission instruction
///
/// ### Accounts:
///   0. `[signer?]` authority - Either this or permissioned_account must be a signer
///   1. `[writable, signer?]` permissioned_account - Either this or authority must be a signer
...
///   3. `[]` magic_program
///   4. `[writable]` magic_context
fn commit_and_undelegate_permission_instruction_impl<'a>(
    ...
    let discriminator_bytes = COMMIT_AND_UNDELEGATE_DISCRIMINATOR.to_le_bytes();
    data_buf[0..8].copy_from_slice(&discriminator_bytes);

    Ok((metas, 5, permission_program_id(), &data_buf[..8]))
}

/// `commit_and_undelegate_permission` CPI accounts using AccountView.
pub struct CommitAndUndelegatePermissionCpiAccountsAccountView<'a> {
...
/// `commit_and_undelegate_permission` CPI instruction using AccountView.
pub struct CommitAndUndelegatePermissionCpiAccountView<'a> {
...
impl<'a> CommitAndUndelegatePermissionCpiAccountView<'a> {
    pub fn new(
        program: &'a AccountView,
        accounts: CommitAndUndelegatePermissionCpiAccountsAccountView<'a>,
    ) -> Self {
        Self {
            __program: program,
    ...
    pub fn invoke_with_remaining_accounts(
        ...
        let (metas, num_accounts, program_id, data) = commit_and_undelegate_permission_instruction_impl(
...
/// Instruction builder for `CommitAndUndelegatePermission` via CPI with AccountView.
///
...
///   4. `[writable]` magic_context
#[derive(Clone, Debug)]
pub struct CommitAndUndelegatePermissionCpiBuilderAccountView<'a> {
    instruction: Box<CommitAndUndelegatePermissionCpiBuilderInstructionAccountView<'a>>,
}

impl<'a> CommitAndUndelegatePermissionCpiBuilderAccountView<'a> {
    pub fn new(program: &'a AccountView) -> Self {
        let instruction = Box::new(CommitAndUndelegatePermissionCpiBuilderInstructionAccountView {
            __program: program,
            authority: None,
    ...
    #[inline(always)]
    pub fn invoke(&self) -> Result<(), ProgramError> {
        let instruction = CommitAndUndelegatePermissionCpiAccountView {
...
#[derive(Clone, Debug)]
struct CommitAndUndelegatePermissionCpiBuilderInstructionAccountView<'a> {
